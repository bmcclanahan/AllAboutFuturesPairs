...
def get_pair_df(symbols, historical_indicators_dfs, pair_online_indicator,
                indicators):
    df1 = historical_indicators_dfs[symbols[0]].set_index('date')
    df2 = historical_indicators_dfs[symbols[1]].set_index('date')
    ...
    data_list = []
    for index in pair_df.index:
        pair_online_indicator.update_indicators(
            pair_df.loc[index].close_diff, pair_df.loc[index].volume,
            pd.Timestamp(index), ohlc=False
        )
        data_list.append(
            [index] + [pair_online_indicator.temp_indicator.__dict__[ind]
            for ind in indicators]
        )
    pair_df = pd.DataFrame(
        data_list, columns=['date', *indicators]
    )
    return pair_df
...
def app_respond(func):
    def wrapper(*args, **kwargs):
        ...
        if (len(args[0]._App__history_and_trade_req_id_params) > 0): # initiate trade after processing all initial history requests
            ...
            if all_symbols_processed:
                args[0].pair_df = get_pair_df(
                    args[0].symbols,
                    args[0].historical_indicators_dfs,
                    args[0].pair_online_indicator,
                    args[0].show_indicators
                )
                if  args[0]._App__trade_mode and not args[0]._App__close_application :
...
class App(Wrapper, Client):
    ...
    def get_historical_data_futures(self, exchange='GLOBEX',
                            ...
                            only_RTH=0, **kwargs):
        all_req_ids = [] # there may be a better way to handle this This is used for the trade_strategy argument functions
                         # could maybe hold parameters based off of some other identifier or no identifier at all
